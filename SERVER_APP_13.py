...
def fetch_companies(query, start_index=0, items_per_page=30):
    """Fetch companies from Companies House API with pagination support."""
    url = f"{BASE_URL}/search/companies?q={query}&start_index={start_index}&items_per_page={items_per_page}"
...
def fetch_all_companies(query):
    ...
    items_per_page = 30  # Number of companies per page (can be adjusted as needed)
    ...
    while True:
        ...
        if data and 'items' in data:
            ...
            # If fewer companies than expected are returned, stop fetching further pages
            if len(data['items']) < items_per_page:
                break  # We've reached the last page
            start_index += items_per_page  # Move to the next page
        else:
            break  # Exit if there's no data or if an error occurs
