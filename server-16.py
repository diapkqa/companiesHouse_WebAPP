...
def get_cache_file(query, page, search_type):
    """Generate a cache file path based on query, page, and search type."""
    cache_key = hashlib.md5(f"{query}-{page}-{search_type}".encode('utf-8')).hexdigest()
    return os.path.join(CACHE_DIR, f"{cache_key}.json")


def fetch_companies(query, page=1, search_type="name"):
    """Fetch companies from Companies House API based on search type."""
    start_index = (page - 1) * LIMIT
    if search_type == "number":
        url = f"{BASE_URL}/company/{query}"
        cache_file = get_cache_file(query, page, search_type)
...
def fetch_company_details(company_number):
    ...
    if response.status_code == 200:
        ...
        if "accounts" in company_data:
            ...
            accounts["AccountsLastStatementDate"] = company_data["accounts"].get("last_accounts", {}).get("made_up_to", "N/A")
...
def search_companies():
    ...
    search_type = request.args.get('type', 'name').strip().lower()  # Added type parameter for flexible searching
    ...
    if search_type not in ['name', 'number', 'sic']:
        return jsonify({"error": "Invalid search type. Use 'name', 'number', or 'sic'."}), 400

    data = fetch_companies(query, page, search_type)
