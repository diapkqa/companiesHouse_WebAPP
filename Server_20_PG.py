...
def fetch_companies(query, page=1):
    ...
    start_index = (page - 1) * LIMIT  # Calculate where the results should start
...
def export_companies():
    """Export companies to an Excel or CSV file."""
    query = request.args.get('query', '').strip()
    ...
    while True:
        ...
        if data and 'items' in data:
            ...
            # Check if there's a next page (use the `links` property to detect pagination)
    ...
    if all_companies:
        ...
        if detailed_companies:
            ...
            return send_file(filepath, as_attachment=True, download_name=filename,
                             mimetype=f'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' if file_type == 'excel' else 'text/csv')
        else:
