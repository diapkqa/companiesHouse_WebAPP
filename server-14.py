...
def fetch_company_details(company_number):
    ...
    if response.status_code == 200:
        ...
        if "confirmation_statement" in company_data:
            confirmation_statement["ConfirmationNextStatementDate"] = company_data["confirmation_statement"].get("next_due", "N/A")
            confirmation_statement["ConfirmationDueDate"] = company_data["confirmation_statement"].get("due_on", "N/A")
            confirmation_statement["ConfirmationLastStatementDate"] = company_data["confirmation_statement"].get("last_made_up_to", "N/A")
...
def search_companies():
    ...
    if search_type == "number":
        ...
        company_details = fetch_company_details(query)
        if company_details:
